...
def scene_reconstruction(mp, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):
        ...
        with torch.no_grad():
            ...
            # if iteration in checkpoint_iterations:
            #     checkpoint_path = os.path.join(scene.model_path, f"chkpnt_{stage}_{iteration}.pth")
            #     print(f"\n[ITER {iteration}] Saving Checkpoint to {checkpoint_path}")
            #     torch.save((gaussians.capture(), iteration), checkpoint_path)

# def prepare_output_and_logger(expname, batch_number):    
#     if not args.model_path:
#         unique_str = expname

#         # Use batch number to differentiate directories for each batch
#         args.model_path = os.path.join("./output/", unique_str, f"batch_{batch_number}_checkpoint")
    
#     # Set up output folder
#     print(f"Output folder for batch {batch_number}: {args.model_path}")
...
def training(model_param, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname, batch=0):
    # first_iter = 0
    tb_writer = prepare_output_and_logger(expname)
    # tb_writer = prepare_output_and_logger(expname, batch_number)

    gaussians = GaussianModel(model_param.sh_degree, hyper)
    model_param.model_path = args.model_path
    timer = Timer()
    scene = Scene(model_param, gaussians, load_coarse=None, batch=batch)
    timer.start()
...
if __name__ == "__main__":
    ...
    torch.cuda.empty_cache()
    torch.backends.cuda.cufft_plan_cache.max_split_size_mb = 16  # Adjust as needed
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[500, 1000, 3000, 4000, 5000, 6000, 7_000, 9000, 10000, 12000, 14000, 20000, 30_000, 45000, 60000])
    ...
    #  # Add new arguments for batch_number and batch_size
    parser.add_argument("--batch_number", type=int, default=0, help="Batch number to use for training data")
    ...
    try:
        ...
        args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.expname, batch=args.batch_number)
    except Exception:
        traceback.print_exc()
        print("cuda error, end training, reexecv program now.")
        os.execv(sys.executable, [sys.executable] + sys.argv)
