    ...
    for i in range(test_times):
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            if i == test_times-1:
             ...
             #   uncertainty.append(rendering['uncertainty'].cpu())# Check if 'uncertainty' key exists
              #  if 'uncertainty' in rendering:
               #     uncertainty.append(rendering['uncertainty'].cpu())
                #else:
                 #   uncertainty.append(torch.zeros_like(rendering["render"]))  # Provide default or placeholder data
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, \
    skip_train : bool, skip_test : bool, skip_video: bool, pc: bool, batch_number: int):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, hyperparam)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False , batch=batch_number)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_number", type=int, default=0, help="Batch number to use for rendering")
    ...
    render_sets(model.extract(args), hyperparam.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.skip_video, args.pc, args.batch_number)
